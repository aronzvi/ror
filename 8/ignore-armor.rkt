...
(struct dragon monster () #:transparent)
...
(define DRAGON (bitmap "graphics/dragon.jpg"))

(define PIC-LIST (list ORC HYDRA SLIME BRIGAND DRAGON))
...
(define DRAGON-IMAGE (overlay DRAGON FRAME))
...
(define (eat w)
  (decrease-attack# w)
  (player-strength+ (orc-world-player w) EATING))

;; OrcWorld -> Void
;; Effect: reduces the number of remaining attacks for this turn
;;   and increases the player's agility level

;(define (stretch w) w) ;stub

(define (stretch w)
  (decrease-attack# w)
  (player-agility+ (orc-world-player w) STRETCHING))

;; OrcWorld -> Void
;; Effect: reduces the number of remaining attacks for this turn
;;   and adds to the player's armor 

;(define (block w) w) ;stub
...
;; Player Nat -> Void
;; effect: decrease player's armor and health. The armor will "absorb" some of the damage

;(define (damage-player-health p delta) (void)) ;stub

(define (damage-player-health p delta)
  (define armor-absorbing-capacity (get-armor-absorbing-capacity p))
  (define damage-to-be-absorbed (min armor-absorbing-capacity (- delta)))
  (define remaining-damage (min (+ delta damage-to-be-absorbed) 0))
  (when (> damage-to-be-absorbed 0)
    (player-armor+ p (- damage-to-be-absorbed)))
  (when (< remaining-damage 0)
    (player-health+ p remaining-damage)))

;; Player -> Nat
;; produces the amount of damage the current armor can absorb
; (define (get-armor-absorbing-capacity p) 0) ;stub

(define (get-armor-absorbing-capacity p)
  (min (player-armor p) ARMOR-ABSORB))
...
(module+ test
  ...
  (check-equal? (let ([p (player 0 0 0 1)])
                  (player-armor+ p 5)
                  p)
                (player 0 0 0 6))
  
  (check-equal? (let ([p (player 5 5 5 5)])
  ...
  (check member 
   ...
   (check-equal? (let ([p (player 5 5 5 5)])
                  (all-monsters-attack-player p (list (dragon 'image 1)))
                  p)
                (player 4 5 5 5))

  (check-equal? (let ([p (player 5 5 5 5)])
                  (damage-player-health p -1)
                  p)
                (player 5 5 5 4))

  (check-equal? (let ([p (player 5 5 5 5)])
                  (damage-player-health p -3)
                  p)
                (player 4 5 5 3))

  (check-equal? (let ([p (player 5 5 5 5)])
                  (damage-player-health p -4)
                  p)
                (player 3 5 5 3))
  ...
  (check-equal? (let ([p (player 5 5 5 0)])
                  (damage-player-health p -2)
                  p)
                (player 3 5 5 0))

  (check-equal?
   (get-armor-absorbing-capacity (player 5 5 5 (add1 ARMOR-ABSORB)))
   ARMOR-ABSORB)

  (check-equal?
   (get-armor-absorbing-capacity (player 5 5 5 0))
   0)

  (check-equal?
   (get-armor-absorbing-capacity (player 5 5 5 ARMOR-ABSORB))
   ARMOR-ABSORB)

  (check-equal?
   (get-armor-absorbing-capacity (player 5 5 5 (sub1 ARMOR-ABSORB)))
   (sub1 ARMOR-ABSORB))
  ...
  (define (prop:monster-attack-player-dec i)
    (test-begin
     (for ([i (in-range i)])
       (define pl (player MAX-HEALTH MAX-AGILITY MAX-STRENGTH MAX-ARMOR))
  ...
  (check-true (>= (let ([p (initialize-player)])
                  ...
                  (let ([p (initialize-player)])
                    ...
                    (player-health p))))
  (check-true (> (player-armor (initialize-player)) 
                 (let ([p (initialize-player)])
  ...
  (check-true (let ([p (initialize-player)])
                ...
                (or (= (player-armor p)
                       (- (player-armor (initialize-player)) 2))
                    (= (player-agility p)
  ...
  (check-equal? (player-strength (orc-world-player
                                  (let ((w (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10 5)]))) 
                                    (eat w)
                                    w)))
                (min MAX-STRENGTH
                     (+ EATING (player-strength (orc-world-player (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10 5)]))))))

  (check-equal? (player-agility (orc-world-player
                                 (let ((w (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10 5)]))) 
                                   (stretch w)
                                   w)))
                (min MAX-AGILITY
                     (+ STRETCHING (player-agility (orc-world-player (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10 5)]))))))

  (check-equal? (player-armor (orc-world-player
                               (let ((w (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10 5)]))) 
                                 (block w)
                                 w)))
                (min MAX-ARMOR
                     (+ BLOCKING (player-armor (orc-world-player (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10 5)]))))))
  ...
  (check-true (lose? (orc-world (player 0 30 30 5) empty 0 0)))
  ...
  (check-true (end-of-orc-battle? (orc-world (player 0 30 30 5) empty 0 0)))
