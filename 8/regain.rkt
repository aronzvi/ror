...
(module+ test
  ...
  (check-equal? (player-strength (orc-world-player
                                  (let ((w (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10)]))) 
                                    (eat w)
                                    w)))
                (min MAX-STRENGTH
                     (+ EATING (player-strength (orc-world-player (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10)]))))))
    ...
    (check-equal? (player-agility (orc-world-player
                                  (let ((w (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10)]))) 
                                    ...
                                    w)))
                (min MAX-AGILITY
                     (+ STRETCHING (player-agility (orc-world-player (struct-copy orc-world (initialize-orc-world) [player (player 35 10 10)]))))))
